...
impl<const N: usize> BitMask<N> {
    ...
    pub fn from_le_bytes(bytes: &[u8], bit_count: usize) -> Result<Self, BitMaskError> {
        ...
        let mut mask = BitMask { data, bit_count };
...
mod tests {
    ...
    fn test_basic_operations() {
        let mut mask: BitMask<4> = BitMask::new(15).unwrap();

        // Test setting individual bits
        mask.set_bit(0, true).unwrap();
        mask.set_bit(7, true).unwrap();
        mask.set_bit(14, true).unwrap();

        assert!(mask.get_bit(0).unwrap());
        assert!(mask.get_bit(7).unwrap());
        assert!(mask.get_bit(14).unwrap());
        assert!(!mask.get_bit(1).unwrap());

        // Test range setting
        mask.set_range(3, 5, true).unwrap();
        for i in 3..=5 {
            assert!(mask.get_bit(i).unwrap());
        }
    }
    ...
    fn test_your_example() {
        ...
        let mut mask: BitMask<2> = BitMask::new(16).unwrap();

        // Set bit 15 (last bit of second byte)
        mask.set_bit(15, true).unwrap();

        // Verify the bytes
        let bytes = mask.as_bytes();
        assert_eq!(bytes.len(), 2);
        assert_eq!(bytes[0], 0); // First byte should be 0
        assert_eq!(bytes[1], 0b10000000); // Second byte should have MSB set

        assert!(mask.get_bit(15).unwrap());
    }

    #[test]
    fn test_endianness() {
        let mut mask: BitMask<4> = BitMask::new(24).unwrap(); // 3 bytes

        // Set some bits: bit 0, bit 8, bit 16
        mask.set_bit(0, true).unwrap(); // LSB of byte 0
        mask.set_bit(8, true).unwrap(); // LSB of byte 1
        mask.set_bit(16, true).unwrap(); // LSB of byte 2
        ...
        let mut mask2: BitMask<4> = BitMask::new(16).unwrap();
        mask2.set_bit(0, true).unwrap(); // 0x01 in byte 0
        mask2.set_bit(15, true).unwrap(); // 0x80 in byte 1
    ...
    fn test_from_bytes() {
        ...
        let mask: BitMask<4> = BitMask::from_le_bytes(&bytes, 20).unwrap();

        assert!(mask.get_bit(0).unwrap()); // LSB of first byte
        assert!(mask.get_bit(15).unwrap()); // MSB of second byte
        assert!(mask.get_bit(16).unwrap()); // LSB of third byte
        assert!(mask.get_bit(17).unwrap()); // Second bit of third byte
        assert!(!mask.get_bit(18).unwrap()); // Should be 0
        assert!(!mask.get_bit(19).unwrap()); // Should be 0

        // Test from_be_bytes
        let be_bytes = [0x03, 0x80, 0x01]; // Same data but big-endian
        let mask_be: BitMask<4> = BitMask::from_be_bytes(&be_bytes, 20).unwrap();
    ...
    fn test_from_arrays() {
        ...
        let mask: BitMask<4> = BitMask::from_le_array(le_array, 16).unwrap();
        ...
        let mask_be: BitMask<4> = BitMask::from_be_array(be_array, 16).unwrap();
    ...
    fn test_unused_bits_cleared() {
        ...
        let mask: BitMask<4> = BitMask::from_le_bytes(&bytes, 12).unwrap(); // Only use 12 bits
    ...
    fn test_roundtrip() {
        ...
        let mut original: BitMask<4> = BitMask::new(20).unwrap();
        original.set_bit(0, true).unwrap();
        original.set_bit(7, true).unwrap();
        original.set_bit(15, true).unwrap();
        original.set_bit(19, true).unwrap();

        // Convert to LE bytes and back
        let le_bytes = original.as_bytes_le();
        let reconstructed: BitMask<4> = BitMask::from_le_bytes(le_bytes, 20).unwrap();
        ...
        for i in 0..20 {
            assert_eq!(
                original.get_bit(i).unwrap(),
                reconstructed.get_bit(i).unwrap()
            );
        }
    }
    ...
    fn test_bitwise_operations() {
        let mut mask1: BitMask<2> = BitMask::new(10).unwrap();
        let mut mask2: BitMask<2> = BitMask::new(10).unwrap();

        mask1.set_range(0, 4, true).unwrap();
        mask2.set_range(2, 6, true).unwrap();

        let and_result = mask1.and(&mask2).unwrap();
        assert!(and_result.get_bit(2).unwrap());
        assert!(and_result.get_bit(3).unwrap());
        assert!(and_result.get_bit(4).unwrap());
        assert!(!and_result.get_bit(0).unwrap());
        assert!(!and_result.get_bit(6).unwrap());
    }
}
